...
public class DataSelectionSort {
    public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        File arquivoMelhor = new File("sort/passwords_data_selectionSort_medioCaso.csv");
        String arquivoMelhorCaso = arquivoMelhor.getAbsolutePath();

        File arquivoPior = new File("sort/passwords_data_selectionSort_melhorCasoDecrescente.csv");
    ...
    public static void medioCaso(String arquivoPassClass) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        // Ordenação pela data completa
        int ordenadas = 0;
        while (ordenadas < linhas - 1) {
            int menor = ordenadas;
            for (int k = ordenadas + 1; k < linhas; k++) {
                if (!passwords[k][2].equals("") && !passwords[menor][2].equals("")) {
                    // Obter as datas como objetos LocalDateTime
                    LocalDateTime data1 = LocalDateTime.parse(passwords[k][3], DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
                    LocalDateTime data2 = LocalDateTime.parse(passwords[menor][3], DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));
    ...
    public static void inverter(String arquivoMelhorCaso) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        while (ordenadas < linhas - 1) {
            ...
            ordenadas++;
            PrintWriter writer = new PrintWriter("sort/passwords_data_selectionSort_melhorCasoDecrescente.csv", "UTF-8");
            writer.print(",password,length,data,class\n");
            // Impressão das senhas ordenadas
            for (i = 0; i < linhas; i++) {
                writer.println(i + "," + passwords[i][1] + "," + passwords[i][2] + "," + passwords[i][3] + "," + passwords[i][4]);
