...
public class DateMonthMergeSort {
    public static void main(String[] args) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        File arquivoMelhor = new File("sort/passwords_data_month_mergeSort_medioCaso.csv");
        String arquivoMelhorCaso = arquivoMelhor.getAbsolutePath();

        File arquivoPior = new File("sort/passwords_data_month_mergeSort_melhorCasoDecrescente.csv");
    ...
    public static void medioCaso(String arquivoPassClass) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        // Ordenação pelo valor do mes
        mergeSortCrescente(passwords, 0, linhas - 1);

        PrintWriter writer = new PrintWriter("sort/passwords_data_month_mergeSort_medioCaso.csv", "UTF-8");
    ...
    public static void melhorCaso(String arquivoMelhorCaso) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        PrintWriter writer = new PrintWriter("sort/passwords_data_month_mergeSort_melhorCaso.csv", "UTF-8");
        writer.print(",password,length,data,class\n");
        // Impressão das senhas ordenadas
        for (i = 0; i < linhas; i++) {
            writer.println(i + "," + passwords[i][1] + "," + passwords[i][2] + "," + passwords[i][3] + "," + passwords[i][4]);
    ...
    public static void inverter(String arquivoMelhorCaso) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        PrintWriter writer = new PrintWriter("sort/passwords_data_month_mergeSort_melhorCasoDecrescente.csv", "UTF-8");
        writer.print(",password,length,data,class\n");
        // Impressão das senhas ordenadas
        for (i = 0; i < linhas; i++) {
            writer.println(i + "," + passwords[i][1] + "," + passwords[i][2] + "," + passwords[i][3] + "," + passwords[i][4]);
    ...
    public static void piorCaso(String arquivoPiorCaso) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        PrintWriter writer = new PrintWriter("sort/passwords_data_month_mergeSort_piorCaso.csv", "UTF-8");
        writer.print(",password,length,data,class\n");
        // Impressão das senhas ordenadas
        for (i = 0; i < linhas; i++) {
            writer.println(i + "," + passwords[i][1] + "," + passwords[i][2] + "," + passwords[i][3] + "," + passwords[i][4]);
    ...
    public static void mergeSortCrescente(String[][] arrayMeses, int esquerda, int direita) {
        if (esquerda < direita) {
            int meio = (esquerda + direita) / 2;
            mergeSortCrescente(arrayMeses, esquerda, meio);
            mergeSortCrescente(arrayMeses, meio + 1, direita);
            mergeCrescente(arrayMeses, esquerda, meio, direita);
        }
    }

    public static void merge(String[][] arrayMeses, int esquerda, int meio, int direita) {
        ...
        for (int i = 0; i < n1; i++) {
            esq[i] = arrayMeses[esquerda + i];
        }
        for (int j = 0; j < n2; j++) {
            dir[j] = arrayMeses[meio + 1 + j];
        }
    ...
    public static void mergeCrescente(String[][] arrayMeses, int esquerda, int meio, int direita) {
        int n1 = meio - esquerda + 1;
        int n2 = direita - meio;
